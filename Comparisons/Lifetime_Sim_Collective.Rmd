...
desired_order.compare<-c("WB Data, WB Model",
...
df.a.compare <- data.frame(
  x = factor(c("Individual Exp Data, Exp Model","Individual WB Data, Exp Model","Mean Exp Data, Exp Model","Mean WB Data, Exp Model",
               ...
               "Mean Inverse Exp Data, Trunc Norm Model", "Mean Inverse WB Data, Trunc Norm Model","WB Data, WB Model")),
...
c.compare.plot <- ggplot(df.c.compare, aes(x, Mean))+
  ...
  geom_hline(yintercept = 0, linetype = "dashed", col = viridis(3)[3])+
  theme_classic()+
  #ylim(-0.55,0.55)+
  coord_flip()
c.compare.plot
...
df.outliers <- df.outliers[, list(x.outliers = c(unlist(df.outliers$x.outliers),rep(NA,250)), y.outliers = c(unlist(df.outliers$y.outliers))),
...
df.outliers.wb<- df.wb |>
  select(Method, x.middle, x.outliers, y.middle, y.outliers) |>
  data.table::data.table()
df.outliers.wb <- df.outliers.wb[, list(x.outliers = unlist(df.outliers.wb$x.outliers), y.outliers = c(unlist(df.outliers.wb$y.outliers),rep(NA, 27))),
                           by = list(Method, x.middle, y.middle)]
...
ggplot(data=dub.wb[1:6,], aes(fill = Method, color = Method)) +
  ...
  labs(title= "Boxplot of Topt and Lifetime",
       x="Topt",
       y="Lifetime")
#ggsave("Topt_Life_WB.png", width = 10)
...
df.outliers.ex <- df.outliers.ex[, list(x.outliers = c(unlist(df.outliers.ex$x.outliers)), y.outliers = c(unlist(df.outliers.ex$y.outliers), rep(NA,2))),
                           by = list(Method, x.middle, y.middle)]
...
ggplot(data=dub.ex[1:5,], aes(fill = Method, color = Method)) +
  ...
  labs(title= "Boxplot of Extreme Temperatures",
       x="Low",
       y="High")
ggsave("Ex_Temp_Exp.png", width = 10)
```
...
medianLow.True.ex.wb<- as.numeric(rep(dub.ex.wb[7,2],5))
medianHigh.True.ex.wb<- as.numeric(rep(dub.ex.wb[7,3],5))
...
df.outliers.ex.wb <- df.outliers.ex.wb[, list(x.outliers = c(unlist(df.outliers.ex.wb$x.outliers), rep(NA,37)), y.outliers = c(unlist(df.outliers.ex.wb$y.outliers))),
                           by = list(Method, x.middle, y.middle)]

ggplot(data=dub.ex.wb[c(1,2,3,4,6),], aes(fill = Method, color = Method)) +
  scale_fill_manual(values = plasma(5))+
  scale_color_manual(values = plasma(5))+
  geom_rect(aes(xmin = q1Low, xmax = q3Low, ymin = q1High, ymax = q3High), alpha = 0.3) +
  geom_rect(aes(xmin = q1Low, xmax = q3Low, ymin = q1High, ymax = q3High), 
            color = "black", fill = NA) +
  geom_segment(aes(x = minLow, y = plot.y.ex.wb$y.middle[c(1,2,3,4,6)], xend = maxLow, yend = plot.y.ex.wb$y.middle[c(1,2,3,4,6)])) + #whiskers
  geom_segment(aes(x = minLow, y = q1High, xend = minLow, yend = q3High)) + #lower end
  geom_segment(aes(x = maxLow, y = q1High, xend = maxLow, yend = q3High)) + #upper end
  geom_segment(aes(x = plot.x.ex.wb$x.middle[c(1,2,3,4,6)], y = minHigh, xend = plot.x.ex.wb$x.middle[c(1,2,3,4,6)], yend = maxHigh)) + #whiskers
  geom_segment(aes(x = q1Low, y = minHigh, xend = q3Low, yend = minHigh)) + #lower end
  geom_segment(aes(x = q1Low, y = maxHigh, xend = q3Low, yend = maxHigh)) + #upper end
  ...
  theme_classic()+
  facet_wrap(dub.ex.wb$Method[c(1,2,3,4,6)],nrow=2)+
  geom_point(aes(x=medianLow.True.ex.wb, y= medianHigh.True.ex.wb),color="black")+
  theme(legend.position="none")+
  labs(title= "Boxplot of Extreme Temperatures",
       x="Low",
       y="High")
ggsave("Ex_Temp_WB.png",width=10)
```
...
plot(wb.wb.10,xlim=c(0,35),ylim=c(0,35),pch=20, col = viridis(5)[1],
...
boxplot.df<-data.frame(
  Method=factor(c(rep("Individual Exp Data, Exp Model",250),rep("Mean Exp Data, Exp Model",250),rep("Individual Inverse Exp Data, Trunc Norm Model",250),rep("Mean Inverse Exp Data, Trunc Norm Model",250),rep("Inverse Mean Exp Data, Trunc Norm Model",250)),levels=rev(c("Inverse Mean Exp Data, Trunc Norm Model","Mean Inverse Exp Data, Trunc Norm Model","Individual Inverse Exp Data, Trunc Norm Model",
...
summary_list.compare <- list(
  xdf1 |> summarize(RMSE = sqrt(mean((trunc.inv - true.value)^2))),
  xdf1_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf6_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf2 |> summarize(RMSE = sqrt(mean((trunc.inv - true.value)^2))),
  xdf2_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf3 |> summarize(RMSE = sqrt(mean((trunc.inv - true.value)^2))),
  xdf3_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf4 |> summarize(RMSE = sqrt(mean((trunc.inv - true.value)^2))),
  xdf4_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf5 |> summarize(RMSE = sqrt(mean((trunc.inv - true.value)^2))),
  xdf5_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2)))
)
...
```{r RMSE.data.wb}
data.rmse.ind.wb <- function(temp, values, a, b, c) {
  curve_value <- exp(-(exp(mean(unlist(lapply(Weibull_param_list1[[1]], function(matrix) matrix[, 1])))) * temp^2 - 
    mean(unlist(lapply(Weibull_param_list1[[2]], function(matrix) matrix[, 1])))* temp + 
    mean(unlist(lapply(Weibull_param_list1[[3]], function(matrix) matrix[, 1])))))
  sqrt(mean((values - curve_value)^2))
}
# Group data by temperature and calculate RMSE for each group
rmse.ind.wb <- bind_rows(Weibull_data.list) %>%
  group_by(T) %>%
  summarise(
    rmse = data.rmse.ind.wb(unique(T), trait, a, b, c)) |> 
    summarise(
    RMSE = sqrt(mean(rmse^2))
  )

data.rmse.mean.wb <- function(temp, values, a, b, c) {
  curve_value <- exp(-(exp(mean(unlist(lapply(Weibull_param_list2[[1]], function(matrix) matrix[, 1])))) * temp^2 - 
    mean(unlist(lapply(Weibull_param_list2[[2]], function(matrix) matrix[, 1])))* temp + 
    mean(unlist(lapply(Weibull_param_list2[[3]], function(matrix) matrix[, 1])))))
  sqrt(mean((values - curve_value)^2))
}
# Group data by temperature and calculate RMSE for each group
rmse.mean.wb <- bind_rows(Weibull_data.list) %>%
  group_by(T) %>%
  summarise(
    rmse = data.rmse.mean.wb(unique(T), trait, a, b, c)) |> 
    summarise(
    RMSE = sqrt(mean(rmse^2))
  )

data.rmse.inv.wb <- function(temp, values, a, b, c) {
  curve_value <- exp(-(exp(mean(unlist(lapply(Weibull_param_list3[[1]], function(matrix) matrix[, 1])))) * temp^2 - 
    mean(unlist(lapply(Weibull_param_list3[[2]], function(matrix) matrix[, 1])))* temp + 
    mean(unlist(lapply(Weibull_param_list3[[3]], function(matrix) matrix[, 1])))))
  sqrt(mean((values - curve_value)^2))
}
# Group data by temperature and calculate RMSE for each group
rmse.inv.wb <- bind_rows(Weibull_data.list) %>%
  group_by(T) %>%
  summarise(
    rmse = data.rmse.inv.wb(unique(T), trait, a, b, c)) |> 
    summarise(
    RMSE = sqrt(mean(rmse^2))
  )

data.rmse.mean.inv.wb <- function(temp, values, a, b, c) {
  curve_value <- exp(-(exp(mean(unlist(lapply(Weibull_param_list4[[1]], function(matrix) matrix[, 1])))) * temp^2 - 
    mean(unlist(lapply(Weibull_param_list4[[2]], function(matrix) matrix[, 1])))* temp + 
    mean(unlist(lapply(Weibull_param_list4[[3]], function(matrix) matrix[, 1])))))
  sqrt(mean((values - curve_value)^2))
}
# Group data by temperature and calculate RMSE for each group
rmse.mean.inv.wb <- bind_rows(Weibull_data.list) %>%
  group_by(T) %>%
  summarise(
    rmse = data.rmse.mean.inv.wb(unique(T), trait, a, b, c)) |> 
    summarise(
    RMSE = sqrt(mean(rmse^2))
  )

data.rmse.inv.mean.wb <- function(temp, values, a, b, c) {
  curve_value <- exp(-(exp(mean(unlist(lapply(Weibull_param_list5[[1]], function(matrix) matrix[, 1])))) * temp^2 - 
    mean(unlist(lapply(Weibull_param_list5[[2]], function(matrix) matrix[, 1])))* temp + 
    mean(unlist(lapply(Weibull_param_list5[[3]], function(matrix) matrix[, 1])))))
  sqrt(mean((values - curve_value)^2))
}
# Group data by temperature and calculate RMSE for each group
rmse.inv.mean.wb <- bind_rows(Weibull_data.list) %>%
  group_by(T) %>%
  summarise(
    rmse = data.rmse.inv.mean.wb(unique(T), trait, a, b, c)) |> 
    summarise(
    RMSE = sqrt(mean(rmse^2))
  )

data.rmse.wb.wb <- function(temp, values, a, b, c) {
  curve_value <- exp(-(exp(mean(unlist(lapply(Weibull_param_list6[[1]], function(matrix) matrix[, 1])))) * temp^2 - 
    mean(unlist(lapply(Weibull_param_list6[[2]], function(matrix) matrix[, 1])))* temp + 
    mean(unlist(lapply(Weibull_param_list6[[3]], function(matrix) matrix[, 1])))))
  sqrt(mean((values - curve_value)^2))
}
# Group data by temperature and calculate RMSE for each group
rmse.wb.wb <- bind_rows(Weibull_data.list) %>%
  group_by(T) %>%
  summarise(
    rmse = data.rmse.wb.wb(unique(T), trait, a, b, c)) |> 
    summarise(
    RMSE = sqrt(mean(rmse^2))
  )
...
comp.plot.1 <- ggplot() +
  ...
  geom_line(data = mean.xdf1_W, aes(y = true_curve, x=point), color = "black", linetype = "dashed")+
    labs(title = "Individual Weibull Data", 
       x = "Temperature", 
       y = "Mortality Rate") +
  theme_minimal()
comp.plot.2 <- ggplot() +
  ...
  geom_line(data = mean.xdf2_W, aes(y = true_curve, x=point), color = "black", linetype = "dashed")+
    labs(title = "Mean Weibull Data", 
       x = "Temperature", 
       y = "Mortality Rate") +
  theme_minimal()
comp.plot.3 <- ggplot() +
  geom_line(data= mean.xdf3_W_N, aes(x = point,y = med.value, color = "Narrow"),lwd=1) +
  geom_ribbon(data = mean.xdf3_W_N, aes(x = point, ymin = lower.hdi, ymax = upper.hdi, fill = "Narrow"), alpha = 0.3) +
  geom_line(data= mean.xdf3_W, aes(x = point,y = med.value, color = "Wide"), lwd=1) +
  geom_ribbon(data = mean.xdf3_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi, fill = "Wide"), alpha = 0.3) +
  geom_line(data = mean.xdf3_W, aes(y = true_curve, x=point), color = "black", linetype = "dashed")+
    labs(title = "Individual Inverse Weibull Data", 
       x = "Temperature", 
       y = "Mortality Rate",
       fill = "95% HDI Bands",
       color = "Posterior Curve") +
...
comp.plot.4 <- ggplot() +
  ...
  geom_line(data = mean.xdf4_W, aes(y = true_curve, x=point), color = "black", linetype = "dashed")+
    labs(title = "Mean Inverse Weibull Data", 
       x = "Temperature", 
       y = "Mortality Rate") +
  theme_minimal()
comp.plot.5 <- ggplot() +
  ...
  geom_line(data = mean.xdf5_W, aes(y = true_curve, x=point), color = "black", linetype = "dashed")+
    labs(title ="Inverse Mean Weibull Data", 
       x = "Temperature", 
       y = "Mortality Rate",
       fill = "95% HDI Bands",
       color = "Posterior Curve") +
...
ggsave("comp.plot.1.png", plot = comp.plot.1)
ggsave("comp.plot.2.png", plot = comp.plot.2)
ggsave("comp.plot.3.png", plot = comp.plot.3, width = 10)
ggsave("comp.plot.4.png", plot = comp.plot.4)
```
...
wb.exp.plot.life <- ggplot() +
  geom_line(data=mean.xdf1_W, aes(x = point, y = avg.value), color = plasma(10)[8], lwd = 1) +
  geom_ribbon(data=mean.xdf1_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = plasma(10)[8], alpha = 0.3) +
  geom_line(data=mean.xdf6_W, aes(x = point, y = avg.value), color = plasma(10)[2], lwd = 1) +
...
gridExtra::grid.arrange(wb.exp.plot.life, wb.exp.plot.mr, nrow = 1, widths = c(1, 1.75))
exp_wb_model_compare <- gridExtra::arrangeGrob(wb.exp.plot.life, wb.exp.plot.mr, nrow = 1, widths = c(1, 1.25))
ggsave("exp_wb_model_compare.png", plot = exp_wb_model_compare, width = 12)
