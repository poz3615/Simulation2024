...
dub$Method <-factor(dub.ex$Method, levels = c("Individual Exp Data, Exp Model","Mean Exp Data, Exp Model", "Individual Inverse Exp Data, Exp Model",
               "Mean Inverse Exp Data, Exp Model", "Inverse Mean Exp Data, Exp Model", "Individual Inverse Exp Data, Exp NT Model",
               "Mean Inverse Exp Data, Exp NT Model", "Inverse Mean Exp Data, Exp NT Model", "True Curve"))
...
df.outliers <- df.outliers[, list(x.outliers = unlist(df.outliers$x.outliers), y.outliers = c(unlist(df.outliers$y.outliers),rep(NA,10))),
...
df.outliers.ex <- df.outliers.ex[, list(x.outliers = c(unlist(df.outliers.ex$x.outliers),rep(NA,18)), y.outliers = c(unlist(df.outliers.ex$y.outliers))),
                           by = list(Method, x.middle, y.middle)]
...
plot(mean.inv.exp.10,pch=20,xlim=c(0,30),ylim=c(0,0.75),
...
```{r RMSE.Boxplot.Exp}
xdf1$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
xdf2$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
xdf3$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
xdf4$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
xdf5$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
xdf3.NT$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
xdf4.NT$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
xdf5.NT$true.value<-rep(true_curve(seq(from = 10, to = 35, length.out = 250)),938)
...
rmse3.NT<-xdf3.NT |> group_by(point) |> summarize(rmse3.NT=sqrt(mean((trunc.inv-1/true.value)^2)))|> select(rmse3.NT)
rmse4.NT<-xdf4.NT |> group_by(point) |> summarize(rmse4.NT=sqrt(mean((trunc.inv-1/true.value)^2)))|> select(rmse4.NT)
rmse5.NT<-xdf5.NT |> group_by(point) |> summarize(rmse5.NT=sqrt(mean((trunc.inv-1/true.value)^2)))|> select(rmse5.NT)
...
boxplot.df<-data.frame(
  ...
  Temperature = rep(seq(from=10,to=35,length.out=250),8)
)
...
```{r RMSE.Boxplot.WB}
xdf1_W$true.value<-rep(my.med(seq(from = 10, to = 35, length.out = 250)),938)
xdf2_W$true.value<-rep(my.med(seq(from = 10, to = 35, length.out = 250)),938)
xdf3_W$true.value<-rep(my.med(seq(from = 10, to = 35, length.out = 250)),938)
xdf4_W$true.value<-rep(my.med(seq(from = 10, to = 35, length.out = 250)),938)
xdf5_W$true.value<-rep(my.med(seq(from = 10, to = 35, length.out = 250)),938)
xdf6_W$true.value<-rep(my.med(seq(from = 10, to = 35, length.out = 250)),938)


rmse1.wb<-xdf1_W |> group_by(point) |> summarize(rmse1.wb=sqrt(mean((trunc.eval-true.value)^2))) |> select(rmse1.wb)
rmse2.wb<-xdf2_W |> group_by(point) |> summarize(rmse2.wb=sqrt(mean((trunc.eval-true.value)^2)))|> select(rmse2.wb)
rmse3.wb<-xdf3_W |> group_by(point) |> summarize(rmse3.wb=sqrt(mean((trunc.eval-true.value)^2)))|> select(rmse3.wb)
rmse4.wb<-xdf4_W |> group_by(point) |> summarize(rmse4.wb=sqrt(mean((trunc.eval-true.value)^2)))|> select(rmse4.wb)
rmse5.wb<-xdf5_W |> group_by(point) |> summarize(rmse5.wb=sqrt(mean((trunc.eval-true.value)^2)))|> select(rmse5.wb)
rmse6.wb<-xdf6_W |> group_by(point) |> summarize(rmse6.wb=sqrt(mean((trunc.eval-true.value)^2)))|> select(rmse6.wb)
...
boxplot.df.wb<-data.frame(
  ...
  Temperature=c(rep(seq(from=10,to=35,length.out=250),6))
)
...
summary_list <- list(
  ...
  xdf3.NT |> summarize(RMSE = sqrt(mean((trunc.inv - 1/true.value)^2))),
  xdf4.NT |> summarize(RMSE = sqrt(mean((trunc.inv - 1/true.value)^2))),
  xdf5.NT |> summarize(RMSE = sqrt(mean((trunc.inv - 1/true.value)^2)))
)
...
RMSE.tab.exp |> ggplot(aes(x=Method, y=RMSE))+
  geom_bar(stat = "identity", position = "dodge", color = "black", fill = "skyblue1")+
...
summary_list.wb <- list(
  xdf1_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf2_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf3_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf4_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf5_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2))),
  xdf6_W |> summarize(RMSE = sqrt(mean((trunc.eval - true.value)^2)))
)
...
RMSE.tab.wb |> ggplot(aes(x=Method, y=RMSE))+
  geom_bar(stat = "identity", position = "dodge", color = "black", fill = "darkseagreen1")+
  geom_text(aes(label = round(RMSE, 2)), hjust = -0.2, position = position_dodge(width = 0.9)) +
...
colnames(RMSE.tab.compare) <- "RMSE"
RMSE.tab.compare
...
ggplot() +
  geom_line(data=N_mean.xdf1, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf1, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf1, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf1, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  ...
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf2, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf2, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf2, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf2, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  ...
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf3, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf3, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf3, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf3, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  ...
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf4, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf4, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf4, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf4, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  ...
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf5, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf5, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf5, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf5, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf5, aes(x = point, y = true_curve), color = "black", linetype = "dashed") +
  ...
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf3_NT, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf3_NT, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf3.NT, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf3.NT, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf3.NT, aes(x = point, y = true_curve), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Inverse NT Data)",
       x = "Temperature",
       y = "Mortality Rate") +
  xlim(13,29)+
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf4_NT, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf4_NT, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf4.NT, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf4.NT, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf4.NT, aes(x = point, y = true_curve), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Mean Inverse NT Data)",
       x = "Temperature",
       y = "Mortality Rate") +
  xlim(13,29)+
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf5_NT, aes(x = point,y = med.value), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf5_NT, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf5.NT, aes(x = point,y = med.value), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf5.NT, aes(ymin = lower.hdi, ymax = upper.hdi, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf5.NT, aes(x = point, y = true_curve), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Inverse Mean NT Data)",
       x = "Temperature",
       y = "Mortality Rate") +
  xlim(13,29)+
  theme_minimal()
...
ggplot() +
  geom_line(data=N_mean.xdf1, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf1, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf1, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf1, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  ...
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf2, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf2, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf2, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf2, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf2, aes(x = point, y = true_curve.inv), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Mean Data)", 
       x = "Temperature", 
       y = "Lifetime") +
  xlim(13,29)+
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf3, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf3, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf3, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf3, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  ...
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf4, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf4, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf4, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf4, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf4, aes(x = point, y = true_curve.inv), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Mean Inverse Data)",
       x = "Temperature",
       y = "Lifetime") +
  xlim(13,29)+
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf5, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf5, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf5, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf5, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf5, aes(x = point, y = true_curve.inv), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Inverse Mean Data)",
       x = "Temperature",
       y = "Lifetime") +
  xlim(13,29)+
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf3_NT, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf3_NT, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf3.NT, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf3.NT, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf3.NT, aes(x = point, y = true_curve.inv), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Inverse NT Data)",
       x = "Temperature",
       y = "Lifetime") +
  xlim(13,29)+
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf4_NT, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf4_NT, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf4.NT, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf4.NT, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf4.NT, aes(x = point, y = true_curve.inv), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Mean Inverse NT Data)",
       x = "Temperature",
       y = "Lifetime") +
  xlim(13,29)+
  theme_minimal()
ggplot() +
  geom_line(data=N_mean.xdf5_NT, aes(x = point,y = med.value.inv), color = "darkslategray2",lwd=1) +
  geom_ribbon(data=N_mean.xdf5_NT, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "darkslategray2", alpha = 0.3) +
  geom_line(data=mean.xdf5.NT, aes(x = point,y = med.value.inv), color = "coral1",lwd=1) +
  geom_ribbon(data=mean.xdf5.NT, aes(ymin = lower.hdi.inv, ymax = upper.hdi.inv, x=point), fill = "coral1", alpha = 0.2) +
  geom_line(data=mean.xdf5.NT, aes(x = point, y = true_curve.inv), color = "black", linetype = "dashed") +
  labs(title = "Narrow and Wide Median Curve Comparison (Inverse Mean NT Data)",
       x = "Temperature",
       y = "Lifetime") +
  xlim(13,29)+
...
```{r n.w.compare.wb.mr}
ggplot() +
  geom_line(data= mean.xdf1_W_N, aes(x = point,y = med.value.inv), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf1_W_N, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf1_W, aes(x = point,y = med.value.inv), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf1_W, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "darkorange1", alpha = 0.3) +
  geom_line(data = mean.xdf1_W, aes(y = true_curve.inv, x=point), color = "black", linetype = "dashed")+
  ...
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf2_W_N, aes(x = point,y = med.value.inv), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf2_W_N, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf2_W, aes(x = point,y = med.value.inv), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf2_W, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "darkorange1", alpha = 0.3) +
  geom_line(data = mean.xdf2_W, aes(y = true_curve.inv, x=point), color = "black", linetype = "dashed")+
  ...
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf3_W_N, aes(x = point,y = med.value.inv), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf3_W_N, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf3_W, aes(x = point,y = med.value), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf3_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "darkorange1", alpha = 0.3) +
  ...
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf4_W_N, aes(x = point,y = med.value.inv), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf4_W_N, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf4_W, aes(x = point,y = med.value.inv), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf4_W, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "darkorange1", alpha = 0.3) +
  geom_line(data = mean.xdf4_W, aes(y = true_curve.inv, x=point), color = "black", linetype = "dashed")+
  ...
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf5_W_N, aes(x = point,y = med.value.inv), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf5_W_N, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf5_W, aes(x = point,y = med.value.inv), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf5_W, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "darkorange1", alpha = 0.3) +
  geom_line(data = mean.xdf5_W, aes(y = true_curve.inv, x=point), color = "black", linetype = "dashed")+
...
```{r n.w.compare.wb.lt}
ggplot() +
  geom_line(data= mean.xdf1_W_N, aes(x = point,y = med.value), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf1_W_N, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf1_W, aes(x = point,y = med.value), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf1_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "darkorange1", alpha = 0.3) +
  ...
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf2_W_N, aes(x = point,y = med.value), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf2_W_N, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf2_W, aes(x = point,y = med.value), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf2_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "darkorange1", alpha = 0.3) +
  ...
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf3_W_N, aes(x = point,y = med.value), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf3_W_N, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf3_W, aes(x = point,y = med.value.inv), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf3_W, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv), fill = "darkorange1", alpha = 0.3) +
  geom_line(data = mean.xdf3_W, aes(y = true_curve.inv, x=point), color = "black", linetype = "dashed")+
    labs(title = "Narrow and Wide Median Curve Comparison (Inverse WB Data)", 
       x = "Temperature", 
       y = "Lifetime") +
  theme_minimal()+
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf4_W_N, aes(x = point,y = med.value), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf4_W_N, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf4_W, aes(x = point,y = med.value), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf4_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "darkorange1", alpha = 0.3) +
  ...
  xlim(13,29)
ggplot() +
  geom_line(data= mean.xdf5_W_N, aes(x = point,y = med.value), color = "greenyellow",lwd=1) +
  geom_ribbon(data = mean.xdf5_W_N, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "greenyellow", alpha = 0.3) +
  geom_line(data= mean.xdf5_W, aes(x = point,y = med.value), color = "darkorange1",lwd=1) +
  geom_ribbon(data = mean.xdf5_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "darkorange1", alpha = 0.3) +
...
nar.wide.rmse.df$Set <- factor(c(rep("Wide",2000),rep("Narrow",2000)))
...
barplot.df.exp$Set <- factor(c(rep("Wide", 8), rep("Narrow", 8)))
barplot.df.exp |> ggplot(aes(x = Method, y = RMSE, fill = Set)) +
  ...
  scale_fill_manual(values = c("Narrow"="blanchedalmond","Wide"="skyblue1")) +
...
barplot.df.wb |> ggplot(aes(x = Method, y = RMSE, fill = Set)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
    geom_text(aes(label = round(RMSE, 4)), hjust = -0.5, position = position_dodge(width = 0.9)) +  
  labs(x = "Method", y = "RMSE") +
  scale_fill_manual(values = c("Narrow"="khaki1","Wide"="darkseagreen1")) +
...
wb.exp.plot <- ggplot() +
  geom_line(data=mean.xdf1_W, aes(x = point, y = avg.value), color = "darkorange1", lwd = 1) +
  geom_ribbon(data=mean.xdf1_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "darkorange1", alpha = 0.3) +
  geom_line(data=mean.xdf6_W, aes(x = point, y = avg.value), color = "hotpink", lwd = 1) +
  geom_ribbon(data=mean.xdf6_W, aes(x = point, ymin = lower.hdi, ymax = upper.hdi), fill = "hotpink", alpha = 0.3) +
...
wb.wb.plot <- ggplot() +
  geom_line(data=mean.xdf1_W, aes(x = point, y = avg.value.inv, color = "Exponential Model"), lwd = 1) +
  geom_ribbon(data=mean.xdf1_W, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv, fill = "Exponential Model"), alpha = 0.3) +
  geom_line(data=mean.xdf6_W, aes(x = point, y = avg.value.inv, color = "Weibull Model"), lwd = 1) +
  geom_ribbon(data=mean.xdf6_W, aes(x = point, ymin = lower.hdi.inv, ymax = upper.hdi.inv, fill = "Weibull Model"), alpha = 0.3) +
  geom_line(data=mean.xdf1_W, aes(x = point, y = true_curve.inv, color = "True Curve"), linetype = "dashed") +
  labs(title = "Mean Curve with Interval Bands and True Curve", 
       x = "Temperature", 
       y = "Mortality Rate",
       color = "Curve", 
       fill = "95% HDI Bands") +
  scale_color_manual(values = c("Exponential Model" = "darkorange", "Weibull Model" = "hotpink", "True Curve" = "black")) +
  scale_fill_manual(values = c("Exponential Model" = "darkorange1", "Weibull Model" = "hotpink")) +
  theme_minimal()

gridExtra::grid.arrange(wb.exp.plot, wb.wb.plot, nrow = 1)
