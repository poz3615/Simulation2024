...
ggplot(data=dub.wb.n[1:6,], aes(fill = Method, color = Method)) +
  ...
  facet_wrap(dub.wb.n$Method[1:6],nrow=2)+
  theme(legend.position="none")+
  labs(title= "Boxplot of Topt and Lifetime",
       x="Topt",
       y="Lifetime")
...
abline(a=0,b=1,lwd=2,col="magenta")
```
...
plot(inv.wb.13.n,xlim=c(0,500),ylim=c(0,0.4),pch=20,
       main="QQ-Plot for Inverse WB Data and Exp Distribution, Narrow Interval",
       xlab="Quantiles of Truncated Normal with Posterior Mean Rate Parameter",
       ylab="Quantiles of Generated Data with True Parameters")
...
xdf1_N$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
xdf2_N$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
xdf3_N$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
xdf4_N$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
xdf5_N$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
xdf3_N.NT$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
xdf4_N.NT$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
xdf5_N.NT$true.value<-rep(true_curve(seq(from= 13, to = 29, length.out = 250)),938)
...
rmse3_N.NT<-xdf3_N.NT |> group_by(point) |> summarize(rmse3.NT=sqrt(mean((trunc.inv-1/true.value)^2)))|> select(rmse3.NT)
rmse4_N.NT<-xdf4_N.NT |> group_by(point) |> summarize(rmse4.NT=sqrt(mean((trunc.inv-1/true.value)^2)))|> select(rmse4.NT)
rmse5_N.NT<-xdf5_N.NT |> group_by(point) |> summarize(rmse5.NT=sqrt(mean((trunc.inv-1/true.value)^2)))|> select(rmse5.NT)
...
boxplot.df_N<-data.frame(
  ...
  Temperature=c(rep(seq(from=13,to=29,length.out=250),8))
)
...
xdf1_W_N$true.value<-rep(my.med(seq(from= 13, to = 29, length.out = 250)),938)
xdf2_W_N$true.value<-rep(my.med(seq(from= 13, to = 29, length.out = 250)),938)
xdf3_W_N$true.value<-rep(my.med(seq(from= 13, to = 29, length.out = 250)),938)
xdf4_W_N$true.value<-rep(my.med(seq(from= 13, to = 29, length.out = 250)),938)
xdf5_W_N$true.value<-rep(my.med(seq(from= 13, to = 29, length.out = 250)),938)
xdf6_W_N$true.value<-rep(my.med(seq(from= 13, to = 29, length.out = 250)),938)
...
boxplot.df.wb_N<-data.frame(
  ...
  Temperature=c(rep(seq(from=13,to=29, length.out = 250),6))
)
...
summary_list_N <- list(
  ...
  xdf3_N.NT |> summarize(RMSE = sqrt(mean((trunc.inv - 1/true.value)^2))),
  xdf4_N.NT |> summarize(RMSE = sqrt(mean((trunc.inv - 1/true.value)^2))),
  xdf5_N.NT |> summarize(RMSE = sqrt(mean((trunc.inv - 1/true.value)^2)))
)
...
RMSE.tab.exp_N$Method <- factor(c(
  ...
  "Individual Inverse Exp Data, Normal Model",
  "Mean Inverse Exp Data, Normal Model",
  "Inverse Mean Exp Data, Normal Model"
), levels = c(
  "Individual Exp Data, Exp Model",
  "Mean Exp Data, Exp Model",
  "Individual Inverse Exp Data, Trunc Norm Model",
...
RMSE.tab.exp_N |> ggplot(aes(x=Method, y=RMSE))+
  geom_bar(stat = "identity", position = "dodge", color = "black", fill = "blanchedalmond")+
...
RMSE.tab.wb_N |> ggplot(aes(x=Method, y=RMSE))+
  geom_bar(stat = "identity", position = "dodge", color = "black", fill = "khaki1")+
  geom_text(aes(label = round(RMSE, 2)), hjust = -0.2, position = position_dodge(width = 0.9)) +
...
row.names(RMSE.tab.compare_N) <- c(
  "Individual Exp Data, Exp Model",
  "Individual WB Data, Exp Model",
  "Individual WB Data, WB Model",
  "Mean Exp Data, Exp Model",
...
colnames(RMSE.tab.compare_N) <- "RMSE"
RMSE.tab.compare_N
