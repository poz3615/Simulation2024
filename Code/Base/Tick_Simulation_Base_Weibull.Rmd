...
dfW<- Weibull_data.list[[100]] 
data <- Weibull_data.list[[100]] 
trait <- data$trait
N.obs <- length(trait)
temp <- data$T
...
lf.fit.mcmc <- as.mcmc(lf.fit.inv.lambda) ## makes an "mcmc" object
closeAllConnections()
...
WB_mean.xdf<-xdf |> group_by(point) |> summarize(avg.value=mean(value),
                                              lower.hdi=hdi(value)[1],
                                              upper.hdi=hdi(value)[2])
WB_true_curve <- function(x) (exp(-(true.exp.a * x^2 - true.exp.b * x + true.exp.c)))
WB_mean.xdf$true_curve <- WB_true_curve(xseq)
ggplot(WB_mean.xdf, aes(x = point)) +
