...
for(i in 1:b){
  ...
  xdf1_N$value.inv <- apply(xdf1_N, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new1_N[i, 4]) * location^2 - exp(df.new1_N[i, 1]) * location + df.new1_N[i, 2])
  })
...
for(i in 1:b){
  ...
  # Apply the quadratic equation for each combination
  xdf2_N$value.inv <- apply(xdf2_N, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new2_N[i, 4]) * location^2 - exp(df.new2_N[i, 1]) * location + df.new2_N[i, 2])
  })
...
for(i in 1:b){
  ...
  # Apply the quadratic equation for each combination
  xdf3_N$value.inv <- apply(xdf3_N, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new3_N[i, 4]) * location^2 - exp(df.new3_N[i, 1]) * location + df.new3_N[i, 2])
  })
...
for(i in 1:b){
  ...
  # Apply the quadratic equation for each combination
  xdf4_N$value.inv <- apply(xdf4_N, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new4_N[i, 4]) * location^2 - exp(df.new4_N[i, 1]) * location + df.new4_N[i, 2])
  })
...
for(i in 1:b){
  ...
  # Apply the quadratic equation for each combination
  xdf5_N$value.inv <- apply(xdf5_N, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new5_N[i, 4]) * location^2 - exp(df.new5_N[i, 1]) * location + df.new5_N[i, 2])
  })
...
for(i in 1:b){
  ...
  # Apply the quadratic equation for each combination
  xdf3_N.NT$value.inv <- apply(xdf3_N.NT, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new3_N.NT[i, 4]) * location^2 - exp(df.new3_N.NT[i, 1]) * location + df.new3_N.NT[i, 2])
  })
...
for(i in 1:b){
  ...
  # Apply the quadratic equation for each combination
  xdf4_N.NT$value.inv <- apply(xdf4_N.NT, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new4_N.NT[i, 4]) * location^2 - exp(df.new4_N.NT[i, 1]) * location + df.new4_N.NT[i, 2])
  })
...
for(i in 1:b){
  ...
  # Apply the quadratic equation for each combination
  xdf5_N.NT$value.inv <- apply(xdf5_N.NT, 1, function(row) {
    i <- row["iteration"]
    location <- row["point"]
    1/(exp(df.new5_N.NT[i, 4]) * location^2 - exp(df.new5_N.NT[i, 1]) * location + df.new5_N.NT[i, 2])
  })
